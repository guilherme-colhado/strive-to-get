...
export const CardHabit = ({element}) => {
    ...
    return<>
        <Card>
            <CardTitle>{element.title} <button onClick={()=>setDelet(true)}><AiOutlineDelete/></button></CardTitle>
            <CardInfo>
                <Infos>
                    <div><span>Categoria:</span> <span>{element.category}</span></div>
                    <div><span>Dificuldade:</span> <span>{element.difficulty}</span></div>
                    <div><span>Frequencia:</span> <span>{element.frequency}</span></div>
                    <div>
                ...
                <button onClick={()=>setArchive(true)}><BiArchiveIn/>Arquivar</button>
            </CardInfo>
        </Card>
        <Modal open={delet}>
            <ComfirmDelete
